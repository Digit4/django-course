...
function caught_speeding(speed, is_birthday){
  var ticket = 0;
  var speed_limits = [60, 80];
  if (is_birthday) {
    for (var i = 0; i < speed_limits.length; i++) {
      speed_limits[i] += 5;
    }
  }
  if (speed <= speed_limits[0]) {
    ticket = 0;
  } else if (speed > speed_limits[0] && speed <= speed_limits[1]) {
    ticket = 1;
  } else {
    ticket = 2;
  }
...
function makeBricks(small, big, goal) {
  if (~~(goal/5) <= big && goal % 5 <= small) { return true; } else { return false;}
}
